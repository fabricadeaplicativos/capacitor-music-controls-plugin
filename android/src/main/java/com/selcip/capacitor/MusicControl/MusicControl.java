...
@NativePlugin()
public class MusicControl extends Plugin {
	...
	public void create(PluginCall call) {
		...
		try {
			final MusicControlsInfo musicControlInfo = new MusicControlsInfo(options);
			System.out.println("MUSIC CONTROL INFO");
			System.out.println(musicControlInfo.getValues());

			final MediaMetadataCompat.Builder metadataBuilder = new MediaMetadataCompat.Builder();

			notification.updateNotification(musicControlInfo);

			// track title
			metadataBuilder.putString(MediaMetadataCompat.METADATA_KEY_TITLE, musicControlInfo.track);
			// artists
			metadataBuilder.putString(MediaMetadataCompat.METADATA_KEY_ARTIST, musicControlInfo.artist);
			// album
			metadataBuilder.putString(MediaMetadataCompat.METADATA_KEY_ALBUM, musicControlInfo.album);

			Bitmap art = getBitmapCover(musicControlInfo.cover);
			if (art != null) {
	...
	public void updateDismissable(PluginCall call) {
		...
		// final JSONObject params = args.getJSONObject(0);
		try {
			final boolean dismissible = params.getBoolean("dismissible");
			this.notification.updateDismissable(dismissible);
			call.success();
		} catch (JSONException e) {
