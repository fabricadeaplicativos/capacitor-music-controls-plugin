...
public class MusicControlsNotification {
	...
	private MusicControlsInfo infos;
	...
	public MusicControlsNotification(Activity cordovaActivity,int id){
		...
		if (Build.VERSION.SDK_INT >= 26) {
			...
			CharSequence name = "capacitor-music-control";
			// The user-visible description of the channel.
			String description = "capacitor-music-control notification";
	...
	public void updateNotification(MusicControlsInfo newInfos){
	...
	// Toggle the dismissible status
	public void updateDismissable(boolean dismissible) {
		if (dismissible == this.infos.dismissible && hasNotification()) {
			return;  // Not recreate the notification with the same data
		}
		this.infos.dismissible =dismissible;
		this.createBuilder();
		this.createNotification();
	}

	// Toggle the dismissible and play/pause status
	public void updateIsPlayingDismissable(boolean isPlaying, boolean dismissible){
		if (dismissible == this.infos.dismissible && isPlaying == this.infos.isPlaying && hasNotification()) {
			return;  // Not recreate the notification with the same data
		}
		this.infos.isPlaying=isPlaying;
		this.infos.dismissible =dismissible;
